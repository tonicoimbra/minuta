-- =============================================================================
-- MIGRA??O CAN?NICA DE MINUTAS
-- Data: 2026-04-24
-- Objetivo: eliminar ambiguidade e manter somente base can?nica ativa.
-- Estrat?gia:
--   1) Backup completo da tabela atual em minuta_templates_backup
--   2) TRUNCATE da minuta_templates
--   3) Carga do sql_minutas_canonico.sql
-- Tudo dentro de uma ?nica transa??o.
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_minuta_templates_backup_step_option
  ON minuta_templates_backup (step_key, option_text);

-- Backup do estado atual (um batch ?nico por execu??o)
WITH batch AS (
...
SELECT
  ...
  t.updated_at,
  t.created_by
FROM minuta_templates t
CROSS JOIN batch;

-- Remove integralmente a base ativa para evitar res?duos/confus?o
TRUNCATE TABLE minuta_templates;

-- =============================================================================
-- IN?CIO DA CARGA CAN?NICA
-- Fonte: sql_minutas_canonico.sql
-- =============================================================================
...
-- =============================================================================
-- FIM DA CARGA CAN?NICA
-- =============================================================================

COMMIT;

-- ROLLBACK OPERACIONAL (manual, se necess?rio):
-- 1) Identifique o ?ltimo backup_batch_id:
--    SELECT backup_batch_id, max(backed_up_at)
--    FROM minuta_templates_backup
...
--    BEGIN;
--    TRUNCATE TABLE minuta_templates;
--    INSERT INTO minuta_templates (id, step_key, option_text, title, template_text, source_files, created_at, updated_at, created_by)
--    SELECT id, step_key, option_text, title, template_text, source_files, created_at, updated_at, created_by
--    FROM minuta_templates_backup
--    WHERE backup_batch_id = '<BATCH_ID_AQUI>';
--    COMMIT;
